...
JLoader::register('ContentHelper', JPATH_ADMINISTRATOR . '/components/com_cjblog/helpers/cjblog.php');

class CjBlogModelUser extends JModelAdmin
{
	protected $text_prefix = 'COM_CJBLOG';

	public $typeAlias = 'com_cjblog.user';
	...
	protected function canDelete ($record)
	{
		if (! empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_cjblog.user.' . (int) $record->id);
		}
	}
	...
	protected function canEditState ($record)
	{
		...
		if (! empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_cjblog.user.' . (int) $record->id);
		}
		// New user, so check against the category.
		elseif (! empty($record->catid))
		{
			return $user->authorise('core.edit.state', 'com_cjblog.category.' . (int) $record->catid);
		}
		// Default to component settings if neither user nor category known.
		else
		{
			return parent::canEditState('com_cjblog');
		}
	}
	...
	public function getTable ($type = 'User', $prefix = 'CjBlogTable', $config = array())
	{
	...
	public function getItem ($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (! empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_cjblog.user');
			}
		}
		...
		if ($assoc)
		{
			...
			if ($item->id != null)
			{
				$associations = JLanguageAssociations::getAssociations('com_cjblog', '#__cjblog_users', 'com_cjblog.item', $item->id);
	...
	{
		// Get the form.
		$form = $this->loadForm('com_cjblog.user', 'user', array(
				'control' => 'jform',
				'load_data' => $loadData
		));
		if (empty($form))
		...
		if ($id != 0 && (! $user->authorise('core.edit.state', 'com_cjblog.user.' . (int) $id)) ||
				 ($id == 0 && ! $user->authorise('core.edit.state', 'com_cjblog')))
		{
	...
	protected function loadFormData ()
	{
		...
		$app = JFactory::getApplication();
		$data = $app->getUserState('com_cjblog.edit.user.data', array());
		...
		if (empty($data))
		{
			...
			{
				$filters = (array) $app->getUserState('com_cjblog.users.filter');
		...
		$this->preprocessData('com_cjblog.user', $data);
		...
		if (parent::save($data))
		{
			...
			if ($assoc)
			{
				...
				$query = $db->getQuery(true)
					...
					->where('context=' . $db->quote('com_cjblog.item'))
				...
				if (! $all_language && count($associations))
				{
					...
					{
						$query->values($id . ',' . $db->quote('com_cjblog.item') . ',' . $db->quote($key));
					}
	...
	public function featured ($pks, $value = 0)
	{
		...
		$table = $this->getTable('Featured', 'CjBlogTable');
	...
	protected function preprocessForm (JForm $form, $data, $group = 'content')
	{
		...
		if ($assoc)
		{
			...
			$fieldset->addAttribute('description', 'COM_CJBLOG_ITEM_ASSOCIATIONS_FIELDSET_DESC');
			$add = false;
