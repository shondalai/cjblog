...
class CjBlogModelReviews extends JModelList
{
	...
	protected function getListQuery ()
	{
		...
		$query->select(
				$this->getState('list.select', 'r.id, r.published, r.reviewed_by, r.review_date, r.remarks, r.checked_out, r.checked_out_time'));
		
		$query->from('#__cjblog_reviews AS r');
		
		// Join over content table
		$query
			->select('a.title, a.alias, a.introtext as description, a.created, a.created_by, a.access, a.publish_up, a.publish_down')
			->join('LEFT', '#__content AS a ON a.id = r.id');
