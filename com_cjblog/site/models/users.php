...
class CjBlogModelUsers extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query
			...
			->join('inner', $db->quoteName('#__cjblog_users') . ' AS cju on a.id = cju.id');

		if($aboutTextApp == 'easyprofile')
		{
			$query
				->select($db->qn($db->escape('e.' . $params->get('easyprofile_about', 'author_info'))).' AS about')
				->join('left', '#__jsn_users AS e on a.id = e.id');
		}
		else
		{
			$query->select('cju.about');
		}
		...
		$query->order($db->escape($this->getState('list.ordering', 'cju.num_articles')) . ' ' . $db->escape($this->getState('list.direction', 'DESC')));
// echo $query->dump();
		return $query;
	}
