	...
	<div class="d-flex">
		...
		<div class="flex-shrink-0 me-3">
			...
			<a href="<?php echo $userProfileUrl;?>" class="thumbnail no-margin-bottom">
				<img class="img-thumbnail" src="<?php echo $api->getUserAvatarImage($avatarApp, $article->created_by, '', 64);?>"
					alt="<?php echo $this->escape($article->author);?>" style="min-width: <?php echo $avatarSize;?>px; max-width: <?php echo $avatarSize;?>px;">
			</a>
			<?php else:?>
			<img class="img-thumbnail" src="<?php echo $api->getUserAvatarImage($avatarApp, $article->created_by, '', $avatarSize);?>"
				 alt="<?php echo $this->escape($article->author);?>" style="min-width: <?php echo $avatarSize;?>px; max-width: <?php echo $avatarSize;?>px;">
			<?php endif;?>
		</div>
		<?php endif;?>
		...
		<div class="flex-grow-1">
			<ul class="list-unstyled list-inline forum-info">
				...
				<li class="list-inline-item"><?php echo JText::sprintf('COM_CJBLOG_ARTICLE_WRITTEN_BY',
						(($profileApp != 'none' && $proUser) ? JHtml::link($userProfileUrl, $article->author) : $article->author))?></li>
				<?php endif;?>
				
				<?php if($params->get('show_category')):?>
				<li class="list-inline-item"><?php echo JText::sprintf('COM_CJBLOG_ARTICLE_CATEGORY',
						JHtml::link(CjBlogHelperRoute::getCategoryRoute($article->catid), $article->category_title));?></li>
				<?php endif;?>
				
				<?php if($params->get('show_date')):?>
				<li class="list-inline-item"><?php echo JText::sprintf('COM_CJBLOG_ARTICLE_DATE', CjLibDateUtils::getHumanReadableDate($article->created));?></li>
				<?php endif;?>
			</ul>
