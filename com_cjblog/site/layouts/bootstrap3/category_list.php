	...
	if ($maxlevel != 0 && count($item->getChildren()) > 0) 
	{
		...
		<div class="row">
	    	...
	    	foreach ($categories as $node)
	    	{
	    		...
	    		{
	    			?>
	    			<div class="col-md-<?php echo $spanMd?> margin-bottom-5 no-margin-left">
						<a href="<?php echo JRoute::_(CjBlogHelperRoute::getCategoryRoute($node));?>">
							<?php echo $this->escape($node->title);?>
							<span class="text-muted visible-xs">(<?php echo JText::plural('COM_CJBLOG_NUM_ARTICLES', $node->numitems);?>)</span>
						</a>
						...
						<small class="text-muted hidden-xs">(<?php echo $node->numitems;?>)</small>
	    			</div>
	    			<?php
	    			$itemNum++;
	    		} 
