...
class PlgCjBlogAppsCjBlog extends JPlugin {
	...
	public function onProfileDisplay( $context, $profile, $params, &$apps ) {
		...
		if ( ! in_array( $apps->id, [ 'summary', 'reputation', 'badges', 'articles' ] ) )
		{
			return true;
		}
	...
	private function loadSummary( $profile, $params, &$apps ) {
		...
		$content = JLayoutHelper::render( $layout . '.profile.summary', [ 'data' => $data ], null, [ 'debug' => false, 'client' => 0, 'component' => 'com_cjblog' ] );
	...
	private function loadReputation( $profile, $params, &$apps ) {
		...
		$content = JLayoutHelper::render( $layout . '.profile.reputation', [ 'data' => $return ], null, [ 'debug' => false, 'client' => 0, 'component' => 'com_cjblog' ] );
	...
	private function loadBadges( $profile, $params, &$apps ) {
		...
		$content = JLayoutHelper::render( $layout . '.profile.badges', [ 'data' => $return ], null, [ 'debug' => false, 'client' => 0, 'component' => 'com_cjblog' ] );
	...
	private function loadArticles( $profile, $params, &$apps ) {
		...
		$content = JLayoutHelper::render( $layout . '.articles_list', [ 'data' => $return ], null, [ 'debug' => false, 'client' => 0, 'component' => 'com_cjblog' ] );
